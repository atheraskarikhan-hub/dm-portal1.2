...
export default async function handler(req, res) {
  ...
  try {
    ...
    debugStep = "Parsing Data Rows Securely";
    ...
    const SD = {
      ...
      counts: { grand: liveStudies.length, vaxTotal: liveStudies.filter(s=>s && (s.vax || '').includes("Vaccine")).length, nvaxTotal: liveStudies.filter(s=>s && (s.vax || '').includes("Non")).length },
